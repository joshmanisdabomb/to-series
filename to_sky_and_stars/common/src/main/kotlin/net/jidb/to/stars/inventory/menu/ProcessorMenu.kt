...
abstract class ProcessorMenu(type: MenuType<*>, id: Int, protected val playerInventory: Inventory, internal val container: Container, internal val data: ContainerData) : RecipeBookMenu(type, id) {
    ...
    override fun handlePlacement(useMaxItems: Boolean, allowDroppingItemsToClear: Boolean, recipe: RecipeHolder<*>, level: ServerLevel, inventory: Inventory): PostPlaceAction {
        return ServerPlaceRecipe.placeRecipe(object : ServerPlaceRecipe.CraftingMenuAccess<ProcessorRecipe> {
            override fun fillCraftSlotsStackedContents(stackedContents: StackedItemContents) {
                this@ProcessorMenu.fillCraftSlotsStackedContents(stackedContents)
            }
            ...
            override fun recipeMatches(recipe: RecipeHolder<ProcessorRecipe>): Boolean {
                return recipe.value().matches(ProcessorRecipeInput((container as ProcessorBlockEntity).blockState, inputSlots.map(container::getItem)), level)
            }
