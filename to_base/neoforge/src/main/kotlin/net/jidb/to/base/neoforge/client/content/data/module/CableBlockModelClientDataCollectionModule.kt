...
open class CableBlockModelClientDataCollectionModule(val center: TexturedModel.Provider, val connection: TexturedModel.Provider, val item: ModelTemplate? = null, val textures: (Identifier) -> TextureMapping = ToBaseModels.TEXTURES_CABLE, val rim: TexturedModel.Provider? = null) : ClientDataCollectionModule() {
    ...
    override fun generateItemModels(collection: DataCollection<Item>, event: ModelClientDataCollectionEvent): Boolean {
        ...
        val cable_item = item.create(
            collection.`object`,
            textures(collection.`object`.identifier.withPrefix("block/")),
            event.item.modelOutput
        )
        event.item.itemModelOutput.accept(collection.`object`, ItemModelUtils.plainModel(cable_item))
        return true
    }
