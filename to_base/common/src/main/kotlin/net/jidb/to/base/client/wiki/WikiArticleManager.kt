package net.jidb.to.base.client.wiki
...
import net.jidb.to.base.wiki.WikiArticleContent
import net.jidb.to.base.wiki.WikiArticleIndex
...
object WikiArticleManager : PreparableReloadListener {
    ...
    override fun reload(sharedState: PreparableReloadListener.SharedState, backgroundExecutor: Executor, barrier: PreparableReloadListener.PreparationBarrier, applyExectutor: Executor): CompletableFuture<Void> {
    ...
    private fun load(resources: Map<Identifier, Resource>): Map<Identifier, WikiArticle> {
        return resources.mapNotNull { (identifier, resource) ->
            try {
                val json = InputStreamReader(resource.open()).use {
                    DATA_GSON.fromJson(it, JsonObject::class.java)
                }
