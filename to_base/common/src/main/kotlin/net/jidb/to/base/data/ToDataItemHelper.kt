...
object ToDataItemHelper {
    ...
    fun <T : Any> getDefaultComponentValue(item: Item, type: DataComponentType<T>, lookup: HolderLookup.Provider? = null): T? {
        ...
        val direct = components[properties]?.filterIsInstance<ComponentEntry<T>>()?.first { it.type == type }?.value
        if (direct != null) {
            return direct
        }

        val delayed = delayedComponents[properties]?.filterIsInstance<DelayedComponentEntry<T>>()?.first { it.type == type }?.value?.create(lookup!!)
        if (delayed != null) {
            return delayed
        }

        val holder = holderComponents[properties]?.filterIsInstance<HolderComponentEntry<T>>()?.first { it.type == type }?.value
        if (holder != null) {
